...
| `<dataset_root>/<animal>/surgery_metadata.yaml` | Forging pipeline (from the animal's latest session)                 | `DatasetData.surgery_paths` (owned by the forging plugin's `/datasets` skill)                         |
...
3. **Resolve the `file_path`** via the owning hand-off — for surgery: session snapshot via
   ...
   `/session-data`), or the dataset per-animal copy via the forging plugin's `/datasets`
...
- [ ] file_path resolved via the right owner (/session-data or /session-discovery for session snapshots,
      forging plugin's /datasets for dataset copies, or the user directly for ad-hoc paths) and is absolute
...
| experiment plugin `/google-sheets-processing` | Owns the reader that captures a read asset from its external source into the on-disk dataclass |
