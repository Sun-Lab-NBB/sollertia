...
declares no field named `incomplete`. The check tests the field **name** only, so it accepts a descriptor that
declares `incomplete` with a different type or a different default, and the `incomplete: bool = True` shape is a
convention the recipes state rather than one the check enforces.
...
contract shapes above. Everything below passes a bare import and fails later, so each one is covered by tests rather
than by a guardrail.
...
| `SESSION_TYPES_USING_VR_TASK` membership       | `required_raw_assets` omits `vr_configuration.yaml`, so session inspection passes a session whose VR snapshot is missing                                 | `tests/data_hierarchy/session_data_test.py`        |
