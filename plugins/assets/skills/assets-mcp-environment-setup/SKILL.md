...
Either command surfaces the second start-time failure as well: a `RuntimeError` raised by the import-time registry
contract checks after an incomplete acquisition-system extension. Hand that case off to `assets:library-extension`,
which owns the registry contracts and the extension procedure.
