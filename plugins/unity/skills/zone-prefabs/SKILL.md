...
| `create_task_tool` fails with "Failed to build segment prefabs."                                | `ConfigLoader.ValidateTemplate` accepts the new `trigger_type` literal but `BuildSegmentPrefabs` has no matching branch and hits its fatal `else`, which names the failing trial in the Unity Console alone | Read the `BuildSegmentPrefabs:` entry through `read_console_tool` to name the trial, then hand off to `/task-generator` for the `trigger_type` branch and the `Place...Zone` helper. The aborted run writes no task prefab, so no zoneless task is left behind |
