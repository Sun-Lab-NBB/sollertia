...
is created, and an edit-phase failure deletes the destination it just wrote, so no partial prefab survives. The new
prefab's root takes the destination filename. The tool enforces the same guarantees this skill applies by hand:
...
- Each `fields` value must land on an int, bool, float, string, or enum property. An enum value must be the member's
  integer ordinal, because `SetSerializedProperty` assigns `property.intValue = Convert.ToInt32(value)`. No shipped
  modifier script declares an enum field, and the root's only enum, `StimulusTriggerZone.triggerMode`, sits out of
  `fields` reach.
...
1. **The new modifier script exists.** Author the new `MonoBehaviour` under `Assets/InfiniteCorridorTask/Scripts/`
   before producing the prefab, because Unity writes the `.cs.meta` file, and the stable GUID inside it, at import time,
   and the prefab YAML needs that GUID to wire the script in. An unfocused Editor never imports a file written outside
   it, so call `refresh_assets_tool` (`/task-scenes`), then poll `get_play_state_tool` out of `compiling`.
...
- **Skipping `inspect_prefab_tool`.** Unity loads a broken prefab into the Editor and reports the import error in the
  Console alone. `inspect_prefab_tool` returns a structured failure on which the agent can act, and `read_console_tool`
  retrieves the import error text behind it.
...
|---------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `clone_zone_prefab_tool` returns "Script type '…' not found"                                                        | The named root or region script is not authored or the project is not compiled                                                                                                                              | Author the script, call `refresh_assets_tool` so the Editor imports it, wait until `get_play_state_tool` stops reporting `compiling`, then re-run the tool                                                                                                                             |
...
| `clone_zone_prefab_tool` returns "Field '…' does not exist on …"                                                    | A `fields` override names a member the modifier script does not declare                                                                                                                                     | Correct the field name to one the script declares, and the failed call deletes the destination, so no partial prefab remains                                                                                                                                                           |
...
| `create_task_tool` fails with "Unable to generate the task. Every segment prefab the template declares must build…" | `ConfigLoader.ValidateTemplate` accepts the new `trigger_type` literal but `BuildSegmentPrefabs` has no matching branch and hits its fatal `else`, which names the failing trial in the Unity Console alone | Read the `Unable to place a trigger zone for trial '…'` entry through `read_console_tool` to name the trial, then hand off to `/task-generator` for the `trigger_type` branch and the `Place...Zone` helper. The aborted run writes no task prefab, so no zoneless task is left behind |
...
| `/task-scenes` (this plugin)                 | Consumer, opens the scene holding the regenerated task, and owns `refresh_assets_tool`                |
...
| `/unity-mcp-environment-setup` (this plugin) | Run first if the Unity Editor bridge is unreachable, and owns `read_console_tool`                     |
