...
Lists, opens, inspects, and enumerates Unity scenes and assets for the `sollertia-virtual-reality` project through the
Unity relay exposed by `slsa mcp`. This skill is the **exclusive** owner of `list_scenes_tool`, `open_scene_tool`,
`inspect_scene_tool`, and `list_assets_tool`. No other skill in the marketplace may call the three scene tools. That
exclusivity binds marketplace skills, not the acquisition runtime: the host VR task driver resolves and opens the task
scene over its own HTTP bridge client during a session (see `experiment:vr-driver-interface`). This skill's asset
enumeration tool (`list_assets_tool`) is read-only and may be called as a **natural share** by any other skill that
...
1. **Verify prerequisites:** Unity Editor running with McpBridge reachable (else `/unity-mcp-environment-setup`).
2. **List scenes:**
   ```text
   list_scenes_tool()
   ```
...
expect `unsaved_changes="save"` to write to every dirty scene file.
...
`open_scene_tool`, `inspect_scene_tool`, or `list_assets_tool`.
