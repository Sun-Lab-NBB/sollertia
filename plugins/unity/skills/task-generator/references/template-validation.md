...
What `ConfigLoader` rejects before `CreateTask` writes anything, and the field-by-field inventory the manual
C#/Python template mirror has to stay in lockstep with. Read this when adding a template field, extending the
validation surface, or diagnosing a `create_task` failure whose message names a template field rather than an asset.

`ConfigLoader.LoadTemplate` deserializes with `UnderscoredNamingConvention` and `IgnoreUnmatchedProperties`, so an
unrecognized YAML key is dropped in silence and surfaces only as the downstream validation failure of whichever
field went missing.
...
The texture-file-existence check resolves `<template dir>/../Textures/<cue.texture>` and is **C#-only** — the Python
`Cue` requires a non-empty filename but cannot see the Unity project.

**Per trial**, in order: non-null entry; trial-key pattern (`^[A-Za-z0-9_]+$`); non-empty `cue_sequence`; every cue
name in the sequence resolves against the cue catalog; `trigger_type` present and one of the five literals;
`occupancy_duration_ms` present for the three occupancy literals; and, whenever a value is supplied at all,
`occupancy_duration_ms` positive and finite (zero is a real duration and an invalid one, so it is rejected on every
trial whatever its trigger type — `null` is how a template says the field is unused).

**Cross-trial:** no two trials may share an identical cue sequence. The signature is the trial's cue names joined by
a single space, and a collision raises
`Trials '<a>' and '<b>' share an identical cue sequence.` Identical sequences are indistinguishable to the host's
cue-stream decomposer, which would silently merge them; use distinct cue codes (textures may be shared) to multiplex
visually identical cues.

**Transitions:** for every trial with a non-empty `transitions` dict — every key must name a defined trial
(`Trial '<name>' has a transition to unknown trial '<target>'.`), every weight must be finite and within `[0.0, 1.0]`
(`... with invalid probability <p>. Must be between 0.0 and 1.0.`), and the weights must sum to `1.0` within
`ProbabilitySumTolerance` (`0.001`) — `Trial '<name>' transition probabilities sum to <s>, must be 1.0.` The sum
accumulates in `double` so a long target list does not drift into the tolerance through single-precision rounding.
`TrialStructure.HasTransitions` treats null and empty identically; either means the runtime samples the next trial
...
Python's `VREnvironment` additionally rejects a non-`int` `segments_per_corridor` (`type(...) is not int`), a
constraint C# cannot express because the field is already typed `int`.
...
`create_task` time, far from the edit that caused it, in the other repo. There is no automated parity check.
...
`VREnvironment` also exposes two C#-only conversion accessors, `CorridorSpacingUnity` and `CueOffsetUnity`, that
divide their cm field by `cmPerUnityUnit`. They are not template fields and have no Python counterpart.
