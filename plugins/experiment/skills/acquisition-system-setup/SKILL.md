...
`list_cameras_tool` is owned by `ataraxis@video:camera-setup`, `list_microcontrollers_tool` by
...
(`ataraxis@video:camera-setup`'s `load_genicam_config_tool`), or — if the live configuration is the new desired
