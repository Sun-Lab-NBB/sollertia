...
The `__all__` list of `cross_system/__init__.py` exports 41 names, and every one of them is available to any
...
| `filesystem_tools.py`     | `probe_writable`                                                                                                                                                                                                               |
...
| YAML plumbing                      | `write_yaml_validated` and `read_yaml` take any `YamlConfig` subclass as their `validator_cls`, while `serialize` accepts any value and `describe_dataclass` any dataclass type (`interfaces/mcp_instance.py`)                                     |
