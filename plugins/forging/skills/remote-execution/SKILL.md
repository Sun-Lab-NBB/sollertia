...
description: >-
  ...
  tracker, and verdict vocabulary that decides what may be done with an outstanding allocation, the remediation that
...
**Does not cover:** compute server credentials, owned by `/server-configuration`; MCP connectivity and the shared
response envelope, owned by `/forging-mcp-environment-setup`; the `slf` surface a job script invokes, owned by
`/cli-reference`; generic batch mechanics, per-tool parameter tables, status presentation, and batch error routing,
owned by `/batch-processing`; the resource model that sizes each allocation, owned by `/job-planning`; the project
manifest and job artifact a remote read mirrors, owned by `/project-state`; dataset hierarchies and state, owned by
`/dataset-definition`; and this machine's session roots, owned by `assets:session-discovery`.

**Handoff rules:** this skill owns everything the server does and everything the transport reports. Route what the
owners above claim, then return here for submission, adoption, resolution, and remediation.
...
functions, open an SSH session yourself, or hand-edit the submission ledger or a processing tracker. Where the tools
...
entry carries no `animal` key and a session entry none for `dataset`, and **the tool rejects no filter**, so confirm
every name against the bare call's `breakdown` before trusting an empty page. The whole tree is read in one server-side
...
**Naming `job_ids` produces a listing and bypasses the account and the date restrictions**, unlike every other read
tool on this server, which stays at its bare stage until a filter is named. They scope the scheduler command itself, so
the account flag and both date bounds are left off, which is the sanctioned way to read an allocation another account
...
every other, and `total_jobs` counts those merged rows before the in-memory filters run while `matched_rows` counts
what they kept.
...
**A read mirrors, then reads the copies.** `interfaces/host_resolution.py::resolve_readable_project` pulls the
project's artifacts into `<working directory>/remote_state/<project>` and hands the read tool that local directory.
...
**A batch leaves the ledger through closure or through explicit remediation.** Four calls snapshot a batch: a status
read closes what its own resolution clears, a remote cancellation closes the batches it cancelled, a submission closes
...
`cancel_reset_and_drop` each hold the batch open for the explicit remediation, while a batch holding no allocation holds
no entry and closes. A failed closure is logged as a warning and leaves its batch outstanding for the next query, so
an identifier retires only where its own closure raised nothing. Remediation runs the same
`close_covered_batches` those three do, then drops the entry whatever its allocations hold.
...
`remote_state/prepared_batches/<batch_id>.outcome.yaml`, **a sibling of that snapshot directory rather than a file
inside it**, since `orchestration/batches.py::_outcome_path` joins the name onto the registry directory itself, and
only then retires the prepared document. A batch absent from the outstanding listing has finished, was remediated, or
was never submitted, and its answer is that outcome file or a `host='remote'` read of the project's job artifact.
...
downstream as `withheld_jobs` rather than running a second copy over a tracker its own executor may still be writing to.
...
`sacct` call and answers `UNRESOLVED` for any allocation it returned no row for. `Server.get_queued_job_ids` issues
`squeue -h -u <user> -o "%i"` and answers the set the controller holds right now, empty for an account holding none.
`remote.py::resolve_tracker_claims` regenerates each covered project's state artifacts on the host before reading them,
once per project and unit kind, because nothing else refreshes them while a batch runs. `read_scheduler_records` reads
accounting first and raises on a failure, since a failed query writes nothing and reading that as "no row for anything"
would resolve every allocation as gone at once; the queue is read second and a failure there is carried into
...
| 2     | This call cancelled it, or accounting answers `BLOCKED`       | `settled`   |
...
is recorded through `SchedulerReading.cancelling` rather than re-queried, since `scancel` is asynchronous.
...
allocation, since `TrackerClaim.unqueryable_executor` marks one neither record can answer for. **The tracker is written
for `stranded` and nothing else**, so `reset_processing_jobs_tool` and `slf reset` stay the way to clear a failure.
Under `force` the verdicts are resolved again after the cancellation, and the `remediation` the response then reports
is composed from what actually ran, reading `cancel_reset_and_drop` only where a cancelled allocation's job was reset.
...
| `stalled`          | None resolves `running` and at least one is `gone` | Remediating. It outlived this read's own closure, so a job is stranded or that closure failed |
...
over all covered batches: monitor on each batch's own `progress` and on `stalled_batch_ids`. The response also carries
`uncovered_batch_ids`, naming any batch another process recorded while the call ran.
...
then runs four steps in this order: cancel, but only under `force`; reset every `stranded` job to `SCHEDULED` through
the host's own reset; snapshot each named batch through the same `close_covered_batches` a settled batch goes through;
drop the ledger entries. `batch_ids` is required and takes no wildcard, since remediation is irreversible.
...
`claim_state` reads `held`, so `force` can `scancel` an allocation another machine submitted and this ledger never
recorded. That is deliberate, since resetting a tracker under the allocation still writing to it is the destruction this
resolution refuses, but it reaches work this host did not start: confirm with the user.
...
refusal by the same route. Under `force` the held allocations are cancelled first, so a live allocation cannot write
...
1. **Notice the stall.** Poll `get_processing_status_tool` with `host='remote'`, or `slf server batches`, and branch on
   `stalled_batch_ids`, never on `active`, which is one flag over every covered batch that a tracker claim alone sets.
...
3. **Clear anything running.** Where a row reads `running`, either wait, or call `cancel_processing_tool` with
   ...
   `running` row neither waiting nor cancelling ever clears: the one whose `claimed_allocation` is empty while
   `tracker_executor_id` names a non-`slurm` scheme, such as the `pid:` a local run writes. No record answers for that
   executor, so no cancellation names it and no elapsed period settles it, and its batch stays `progressing` rather than
   joining `stalled_batch_ids`. Confirm with the user that nothing is still writing to that unit, then reset that job
   with `reset_processing_jobs_tool`: its verdict then reads `abandoned`, whose remediation is the plain `drop`, so the
   next status read closes the batch on its own.
4. **Remediate.** Call `retire_remote_batches_tool(batch_ids=[...])`, or run `slf server retire-batch -b <id>`, adding
   `force=True` only after step 3 and `drop_without_outcome=True` only after a snapshot has actually failed.
5. **Confirm what it did.** Read `reset_jobs`, each `allocations[*]` entry's `cancelled`, `tracker_reset`,
...
6. **Re-run the released work.** Prepare and execute the same pipeline and units again through `/batch-processing`.
   ...
   adopts it, since the reset cleared both the running status and the executor identifier its claim lived on.

A job whose verdict read `failed` keeps that record deliberately: step 6 prepares and reruns it as outstanding work, and
...
`force` that held claim refuses the whole batch, and with `force` the allocation is cancelled and the job reset here. A
job running under a non-`slurm` executor is the one case `force` does not release: it cancels and resets nothing, so the
ledger entry goes while the tracker keeps claiming the run and every later batch withholds that job and its dependents
again. Reset it through step 3 first.
...
The name leads with the job's submission index zero-padded to four digits, joins the unit name, the job name, and the
specifier with hyphens, replaces every character outside `A-Za-z0-9._-` with an underscore, and is carried by the
script and both logs. The two conditional directives appear together whenever this submission holds an allocation for
one of the job's prerequisites, newly queued or adopted, while a prerequisite that already succeeded stays on the
descriptor and contributes no dependency. Every upstream identifier joins one `afterok:` prefix separated by colons,
the kill directive turns an unsatisfiable dependency into a terminal state a status query can report, and no other
...
emitted after the preamble, and the script removes itself through an exit trap so its exit status stays the work's.
**Each script runs the same `slf` command a local run would**, from the dispatch table both backends share, though a
local dispatch also caps each job's width at what this machine's core budget supplies, so prove a pipeline on one
session locally before a project-wide remote batch.
...
- [ ] Any running allocation was waited out or cancelled before force was considered, the user confirmed that force
