...
description: >-
  ...
  session paths from `assets:session-discovery`.
...
1. **Discover sessions** — Use `assets:session-discovery` to obtain confirmed session paths.
...
| `Source path does not exist`                       | Verify path; re-run `assets:session-discovery` |
