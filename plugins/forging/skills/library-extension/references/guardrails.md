...
| The sizing model, its routing branch, and its mapped term             | Stage, pipeline | `ValueError` during the sizing pass, or an allocation reserved below what a job that maps its input holds resident       | The orchestration footprint tests              |
