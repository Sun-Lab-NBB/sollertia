...
- The forged column roster and the per-column descriptions. Owned by `mesoscope:mesoscope-vr-processing-schema`.
...
- Mesoscope-frame TTL feather production. Owned by `mesoscope:mesoscope-vr-module-parsing`.
...
`forging:data-processing-design`). This skill documents only the Mesoscope-VR-specific content of the fluorescence
sub-assembly, the alignment algorithm and its constants, not the orchestration that schedules it.
...
`mesoscope:mesoscope-vr-dataset-assembly`. `assemble_experiment_dataset` runs this sub-assembly first and takes
...
The authoritative target frame count comes from the single-recording `RecordingArrays.CELL_FLUORESCENCE` array,
located by `resolve_array_path`. The function reads its shape through a memory-mapped header read
...
the TTL module parser, see `mesoscope:mesoscope-vr-module-parsing`) holds `time_us` and `ttl_state` columns, one row
...
`_match_runs_to_acquisitions` returns `None` when there are no acquisitions, or when there are more acquisitions
than runs. Otherwise it brute-forces every `permutations(range(acquisition_count))`, delegating each ordering to the
...
across several runs, and the match is approximate rather than exact because an acquisition emits a small surplus
from scanner arming minus the pulses the duration filter rejected. `_search_run_spans` abandons any branch whose
accumulated cost already matches the cheapest complete assignment, and stops extending a span once its total passes
the target. The search returns `None` when the cheapest assignment still carries `_UNMATCHED_COST`.
...
derived within-session display column. See `mesoscope:mesoscope-vr-dataset-assembly`.
...
| `mesoscope:mesoscope-vr-dataset-assembly`  | Calls this sub-assembly and consumes the reference clock it produces                |
| `mesoscope:mesoscope-vr-module-parsing`    | Produces the upstream mesoscope-frame TTL feather                                   |
| `mesoscope:mesoscope-vr-processing-schema` | Owns the DatasetColumn roster and the processed-data filename roster                |
