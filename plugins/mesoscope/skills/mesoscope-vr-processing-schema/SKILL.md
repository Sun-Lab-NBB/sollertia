...
- The runtime feather schemas and the trial decomposition that fills them. Owned by
...
- The assembly algorithms that compute the `DatasetColumn` values, the sub-dataset concatenation, and the clipping
  applied afterwards. Owned by `mesoscope:mesoscope-vr-dataset-assembly`.
- The pupil-metric computation behind the face-camera pupil feather. Owned by `mesoscope:mesoscope-vr-video-tracking`.
...
Mesoscope-VR parsers write and most of which the donated assembly worker reads back. There are two output homes, not
...
maximum UInt8). The masking pass that assigns them belongs to `mesoscope:mesoscope-vr-dataset-assembly`.
...
Most of the roster is conditional. Only six behavior columns are present in every forged session, so any consumer
reading a column outside that set must first confirm the column exists in the loaded frame. The `DatasetColumn` class
docstring states the full condition set.
...
producing or consuming stage. This skill owns all three, and the producing and consuming skills reference here.
...
| Skill                                           | Relationship                                                                        |
|-------------------------------------------------|---------------------------------------------------------------------------------------|
| `mesoscope:mesoscope-vr-module-parsing`         | Producer of the eight microcontroller feathers and owner of their column schemas    |
| `mesoscope:mesoscope-vr-trial-decomposition`    | Producer of the seven runtime feathers and owner of their column schemas            |
| `mesoscope:mesoscope-vr-video-tracking`         | Producer of the face-camera pupil feather and owner of the pupil-metric definitions |
| `mesoscope:mesoscope-vr-fluorescence-alignment` | Consumer of `MESOSCOPE_FRAME`, producer of the cindra fluorescence columns          |
| `mesoscope:mesoscope-vr-dataset-assembly`       | Consumer: assembles the roster feathers into the `DatasetColumn` set of `data.feather` |
